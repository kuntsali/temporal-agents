...
import { apiService, storageService, ApiError } from "../services/api";

const POLL_INTERVAL = 600;
const INITIAL_ERROR_STATE = { visible: false, message: "" };

const useStoredWorkflowId = () => {
    const [workflowId, setWorkflowIdState] = useState(() => storageService.getWorkflowId());

    const setWorkflowId = useCallback((id) => {
        storageService.setWorkflowId(id);
...
export default function App() {
    ...
    const handleError = useCallback((err, context) => {
        ...
        const isConversationFetchError = err instanceof ApiError && err.status === 404;
    ...
    const fetchConversationHistory = useCallback(async () => {
        try {
            ...
            const data = await apiService.getConversationHistory(activeWorkflowId);
            ...
            if (newConversation.length > 0) {
                ...
                const actor = lastMsg.actor || lastMsg.type;
                const isAgentMessage = actor === "agent";

                setLoading(!isAgentMessage);
                const rawResponse = lastMsg?.response;
                const nextValue = rawResponse && typeof rawResponse === 'object'
                    ? rawResponse.next
                    : null;
                setDone(nextValue === 'done');

                setLastMessage((prevLastMessage) =>
                    !prevLastMessage || JSON.stringify(lastMsg) !== JSON.stringify(prevLastMessage)
                        ? lastMsg
                        : prevLastMessage
                );
            } else {
            ...
            if (err instanceof ApiError && err.status === 404) {
                ...
                try {
                    ...
                    handleError(resetError, "resetting session");
                }
            } else {
                handleError(err, "fetching conversation");
            }
        }
    }, [ensureWorkflow, clearErrorOnSuccess, handleError, resetWorkflow]);

    useEffect(() => {
        ensureWorkflow().catch((err) => handleError(err, "initializing session"));
    }, [ensureWorkflow, handleError]);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            setError(INITIAL_ERROR_STATE);
            const activeWorkflowId = await ensureWorkflow();
            await apiService.sendMessage(activeWorkflowId, trimmedInput);
            setUserInput("");
        } catch (err) {
            if (err instanceof ApiError && err.status === 404) {
                try {
                    const newWorkflowId = await resetWorkflow();
                    if (newWorkflowId) {
                        await apiService.sendMessage(newWorkflowId, trimmedInput);
                        setUserInput("");
                    }
                } catch (resetError) {
                    handleError(resetError, "sending message");
                    setLoading(false);
                }
            } else {
    ...
    const handleConfirm = async () => {
        try {
            ...
            setError(INITIAL_ERROR_STATE);
            const activeWorkflowId = await ensureWorkflow();
            await apiService.confirm(activeWorkflowId);
        } catch (err) {
            if (err instanceof ApiError && err.status === 404) {
                try {
                    const newWorkflowId = await resetWorkflow();
                    if (newWorkflowId) {
                        await apiService.confirm(newWorkflowId);
                    }
                } catch (resetError) {
                    handleError(resetError, "confirming action");
                    setLoading(false);
                }
            } else {
    ...
    const handleStartNewChat = async () => {
        try {
            ...
            setLoading(true);
            if (workflowId) {
                await apiService.endConversation(workflowId);
            }
            await resetWorkflow();
            setConversation([]);
            setLastMessage(null);
            ...
            <div className="flex-grow flex justify-center px-4 py-2 overflow-hidden">
                <div className="w-full max-w-lg bg-white dark:bg-gray-900 p-8 px-3 rounded shadow-md
                    ...
                    <div ref={containerRef}
                        ...
                        />
                        {done && (
                            <div className="text-center text-sm text-gray-500 dark:text-gray-400 mt-4 animate-fade-in">
                                Chat ended
                            </div>
                        )}
                    </div>
